  ...
  try {
    ...
    if (!clientId || !artistId)
      return res.status(400).json({
        success: false,
        message: "clientId and artistId are required",
      });
...
exports.listMyChats = async (req, res) => {
  try {
    ...
    const chats = await Chat.find({
      $or: [{ client: userId }, { artist: userId }],
    })
      .sort({ updatedAt: -1 })
      .populate("client", "firstName lastName userType")
      .populate("artist", "firstName lastName userType");
    ...
    const chatsWithUnread = chats.map((chat) => {
      const unreadCount = chat.messages.filter((msg) => {
        // Message is unread if current user hasn't read it and didn't send it
        const isUnread = !(msg.readBy || []).some(
          (id) => String(id) === String(userId)
        );
        const isNotOwnMessage = String(msg.sender) !== String(userId);
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (!text && (!attachments || attachments.length === 0)) {
      return res
        .status(400)
        .json({ success: false, message: "text or attachments are required" });
    }
    ...
    try {
      const artistHasChat = await User.findOne({
        _id: chat.artist._id,
        chatIds: chat._id,
      });
      if (!artistHasChat) {
        await User.findByIdAndUpdate(chat.artist._id, {
          $addToSet: { chatIds: chat._id },
        });
      }
    } catch (_) {}
...
exports.markRead = async (req, res) => {
  try {
    ...
    if (hasUpdates) {
      // Use findByIdAndUpdate with runValidators: false to avoid validation errors on existing messages
