...
const EmailCheck = () => {
  ...
  const handleContinueAfterVerification = async () => {
    ...
    try {
      ...
      await authAPI.resendVerificationEmail(email);
      // Not verified yet
      setError("Your email isn't verified yet. Please check your inbox and try again.");
    } catch (e) {
      if ((e.message || '').toLowerCase().includes('already verified')) {
        setVerified(true);
      } else {
  ...
  return (
    <>
      ...
      <div className="auth-container" style={{ margin: '0 auto', height: '100vh'}}>
        <div className="auth-card" style={{ maxWidth: 520, animation: 'fadeIn 420ms ease-out both' }}>
          {!verified ? (
            <div style={{ textAlign: 'center', padding: '1.5rem 1rem' }}>
              ...
              <h2 style={{ color: '#7c2d12', margin: '0 0 0.5rem' }}>Check Your Email</h2>
              <p style={{ color: '#7c2d12', margin: 0, lineHeight: 1.6 }}>
                We've sent a verification link to <strong>{email || 'your@email.com'}</strong>.
              ...
              <div style={{ marginTop: '1.25rem' }}>
                ...
                <div style={{ marginTop: '0.75rem' }}>
                  <Link to="/signup" style={{ color: '#7c2d12', textDecoration: 'underline' }}>Use a different email</Link>
                </div>
              </div>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: '1.5rem 1rem', animation: 'fadeIn 420ms ease-out both' }}>
              <div style={{
                width: 64,
                height: 64,
                margin: '0 auto 1rem',
              ...
              <h2 style={{ color: '#6b5544', margin: '0 0 0.5rem' }}>Email Verified!</h2>
              <p style={{ color: '#6b5544', margin: 0, lineHeight: 1.6 }}>
                Your email is confirmed. You can now continue to phone verification.
              </p>
