            ...
            return (
              <div key={i} style={{
                ...
                <button onClick={() => setOpenIdx(isOpen ? null : i)} style={{
                  ...
                  background: 'transparent', border: 'none', padding: '18px 20px', cursor: 'pointer', color: '#4A2C1D', fontWeight: 700
                }}>
                  <span>{item.q}</span>
                  {isOpen ? <FaChevronUp color="#A4693D" /> : <FaChevronDown color="#A4693D" />}
                </button>
                <div style={{ maxHeight: isOpen ? 400 : 0, overflow: 'hidden', transition: 'max-height .35s ease' }}>
                  <div style={{ padding: '0 20px 18px', color: '#654C3F' }}>{item.a}</div>
                </div>
              </div>
            );
          })}
